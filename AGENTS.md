...
- **Frontend** — **React 19 + TypeScript**, **Vite**, package manager **pnpm**; **Mantine** (`@mantine/core` — the single style foundation is `src/theme.ts`), **react-feather** icons, **i18n** via **react-i18next** (+ `i18next-browser-languagedetector`; catalogs under `src/i18n/`, **ZH + EN**), **PWA** via `vite-plugin-pwa`; **`@zxing/browser`** + **`@zxing/library`** (client-side 1D + 2D barcode decoding, M5). Quality tools: **eslint**, **tsc**, **vitest**. **Wire/display split** (ratified M1.5): the backend speaks **stable error codes** and emits no display text; the **frontend owns all human-facing strings** and localizes codes via the `errors` namespace.
